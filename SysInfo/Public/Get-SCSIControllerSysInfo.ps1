function Get-SCSIControllerSysInfo 
{
  <#
      ...
      PS C:\> Get-SCSIControllerSysInfo -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-SCSIControllerSysInfo -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  [Collections.ArrayList]$DefaultProperties = 'Name', 'DriverName', 'Status', 'StatusInfo', 'ProtocolSupported', 'Manufacturer', 'SystemName'
  ...
  $SCSIController = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
  ...
  foreach ($_ in $SCSIController)
  {
    ...
    $_.ProtectionManagement = Get-ProtectionManagement -Code ($_.ProtectionManagement)
