function Get-OpticalSensor 
{
  <#
      ...
      PS C:\> Get-OpticalSensor -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-OpticalSensor -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  function Get-OpticalSensorOnly
  {
    <#
        .SYNOPSIS
        Filters Optincal Sensor only.
    #>


    param
    (
      [Parameter(Mandatory = $true, ValueFromPipeline = $true, HelpMessage = 'Filter Optical Sensor Only')]
      $InputObject
    )
    process
    {
      if ($InputObject.PointingType -eq 'Optical Sensor')
      {
        $InputObject
      }
    }
  ...
  if ($Authentication -eq '') 
  {
    if ($Protocol -eq '') 
    {
      $OpticalSensor = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Properties $Properties | Get-OpticalSensorOnly
    }
    else 
    {
      $OpticalSensor = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Protocol $Protocol -Properties $Properties | Get-OpticalSensorOnly
    }
  }
  else 
  {
    if ($Protocol -eq '') 
    {
      $OpticalSensor = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Properties $Properties | Get-OpticalSensorOnly
    }
    else 
    {
      $OpticalSensor = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $Properties | Get-OpticalSensorOnly
    }
  }

  Write-Output -InputObject $OpticalSensor
}
