function Get-Mouse 
{
  <#
      ...
      PS C:\> Get-Mouse -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-Mouse -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  function Get-MouseOnly
  {
    ...
    param
    (
      [Parameter(Mandatory = $true, ValueFromPipeline = $true, HelpMessage = 'Filter Mouse Only')]
      $InputObject
    )
    process
    {
      if ($InputObject.PointingType -eq 'Mouse')
      {
        $InputObject
      }
    }
  ...
  if ($Authentication -eq '') 
  {
    if ($Protocol -eq '') 
    {
      $Mouse = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Properties $Properties | Get-MouseOnly
    }
    else 
    {
      $Mouse = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Protocol $Protocol -Properties $Properties | Get-MouseOnly
    }
  }
  else 
  {
    if ($Protocol -eq '') 
    {
      $Mouse = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Properties $Properties | Get-MouseOnly
    }
    else 
    {
      $Mouse = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $Properties | Get-MouseOnly
    }
  }

  Write-Output -InputObject $Mouse
}
