function Get-DesktopMonitor 
{
  <#
      ...
      PS C:\> Get-DesktopMonitor -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-DesktopMonitor -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  [Collections.ArrayList]$DefaultProperties = 'DeviceID', 'Name', 'DisplayType', 'MonitorManufacturer', 'ScreenHeight', 'ScreenWidth', 'SystemName'
  ...
  $DesktopMonitor = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
  ...
  foreach ($_ in $DesktopMonitor)
  {
    ...
    $_.DisplayType = Get-DisplayType -Code ($_.DisplayType)
