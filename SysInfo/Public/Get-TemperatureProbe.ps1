function Get-TemperatureProbe 
{
  <#
      ...
      PS C:\> Get-TemperatureProbe -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-TemperatureProbe -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  [Collections.ArrayList]$DefaultProperties = 'CurrentReading', 'Name', 'Description', 'MinReadable', 'MaxReadable', 'Status', 'SystemName'
  ...
  $TemperatureProbe = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
