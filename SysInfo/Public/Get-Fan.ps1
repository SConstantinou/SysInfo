function Get-Fan 
{
  <#
      ...
      PS C:\> Get-Fan -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-Fan -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  $ClassName = 'Win32_Fan'
  [Collections.ArrayList]$DefaultProperties = 'Name', 'ActiveCooling', 'DesiredSpeed', 'VariableSpeed', 'Status', 'SystemName'
  ...
  $Fan = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
