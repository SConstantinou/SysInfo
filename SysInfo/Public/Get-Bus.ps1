function Get-Bus 
{
  <#
      ...
      PS C:\> Get-Bus -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-Bus -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  $ClassName = 'Win32_Bus'
  [Collections.ArrayList]$DefaultProperties = 'Name', 'DeviceID', 'BusNum', 'BusType', 'SystemName'
  ...
  $Bus = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
  ...
  foreach ($_ in $Bus)
  {
    ...
    $_.BusType = Get-BusType -Code ($_.BusType)
  }
