function Get-SoundDevice 
{
  <#
      ...
      PS C:\> Get-SoundDevice -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-SoundDevice -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  [Collections.ArrayList]$DefaultProperties = 'Manufacturer', 'Name', 'Status', 'StatusInfo', 'SystemName'
  ...
  $SoundDevice = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
