function Get-GlidePoint 
{
  <#
      ...
      PS C:\> Get-GlidePoint -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-GlidePoint -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  function Get-GlidePointOnly
  {
    <#
        .SYNOPSIS
        Filters GlidePoint only.
    #>


    param
    (
      [Parameter(Mandatory = $true, ValueFromPipeline = $true, HelpMessage = 'Filters only GlidePoint')]
      $InputObject
    )
    process
    {
      if ($InputObject.PointingType -eq 'Glide Point')
      {
        $InputObject
      }
    }
  ...
  if ($Authentication -eq '') 
  {
    if ($Protocol -eq '') 
    {
      $GlidePoint = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Properties $Properties | Get-GlidePointOnly
    }
    else 
    {
      $GlidePoint = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Protocol $Protocol -Properties $Properties | Get-GlidePointOnly
    }
  }
  else 
  {
    if ($Protocol -eq '') 
    {
      $GlidePoint = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Properties $Properties | Get-GlidePointOnly
    }
    else
    {
      $GlidePoint = Get-PointingDevice -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $Properties | Get-GlidePointOnly
    }
  }

  Write-Output -InputObject $GlidePoint
}
