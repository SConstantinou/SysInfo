function Get-MemoryArray 
{
  <#
      ...
      PS C:\> Get-MemoryArray -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-MemoryArray -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  $MemoryArray = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
