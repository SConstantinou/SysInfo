function Get-Refrigeration 
{
  <#
      ...
      PS C:\> Get-Refrigeration -ComputerName Server1 -Credential domain\user

      This command gets the information from Server1 using a different user

      .EXAMPLE

      PS C:\> Get-Refrigeration -ComputerName Server1 -Credential domain\user -Authentication Basic
  ...
  [Collections.ArrayList]$DefaultProperties = 'Name', 'Status', 'SystemName'
  ...
  $Refrigeration = Get-Info -ClassName $ClassName -ComputerName $ComputerName -Credential $Credential -Authentication $Authentication -Protocol $Protocol -Properties $AllProperties
