function Get-PhysicalMedia {
    ...
    $Properties = 'Caption','Description','InstallDate','Name','Status','Manufacturer','Model', 
        'OtherIdentifyingInfo','PartNumber','PoweredOn','SerialNumber','SKU','Tag',
        'Version','HotSwappable','Removable','Replaceable','Capacity','CleanerMedia',
        'MediaDescription','MediaType','WriteProtectOn'

    $PhysicalMedia = Get-CimInstance -ClassName Win32_PhysicalMedia -Property $Properties | Select-Object $Properties
    $PhysicalMedia | ForEach-Object {$_.MediaType = $MediaType["$($_.MediaType)"]}
